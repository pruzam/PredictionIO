...
class MahoutModelConstructorSpec extends Specification {
  ...
  "LTMahoutModelConstructor" should {

    val inputDir = "/tmp/pio_test/lt/"
    ...
    TestUtils.writeToFile(ratingsCSV, s"${inputDir}ratings.csv")
    TestUtils.writeToFile(predicted, s"${inputDir}predicted.tsv")
    TestUtils.writeToFile(closetCSV, s"${inputDir}closet.csv")
    ...
    "correctly writes ItemRecScores with larger numRecommendations" in {
      ...
      val u2Expected = ItemRecScore(
        uid = "u2",
        iids = Seq("i2", "i0", "i1", "i4", "i3"),
        scores = Seq(22.5, 4.5, 3.3, 2.2, 2.2),
        itypes = Seq(Seq("t2", "t3"), Seq("t1", "t2"), Seq("t1"), Seq("t2", "t3"), Seq("t3")),
        appid = appid,
        algoid = algoid,
        modelset = modelSet)

      LTMahoutModelConstructor.main(TestUtils.argMapToArray(args))
    ...
    "correctly writes ItemRecScores with subset itemsIndex.tsv" in {
      ...
      TestUtils.writeToFile(ratingsCSV, s"${inputDir}ratings.csv")
      TestUtils.writeToFile(predicted, s"${inputDir}predicted.tsv")
      TestUtils.writeToFile(List(), s"${inputDir}closet.csv")
    ...
    "correctly writes ItemRecScores with unseenOnly=true" in {
      ...
      val u2Expected = ItemRecScore(
        uid = "u2",
        iids = Seq("i2", "i0", "i4", "i3"),
        scores = Seq(22.5, 4.5, 2.2, 2.2),
        itypes = Seq(Seq("t2", "t3"), Seq("t1", "t2"), Seq("t2", "t3"), Seq("t3")),
        appid = appid,
        algoid = algoid,
        modelset = modelSet)

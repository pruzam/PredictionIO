 ...
 *
 */
object LTMahoutModelConstructor {
  /* global */
  ...
  def modelCon(arg: JobArg) = {
    ...
    // Handle closet items
    val closetMap: Map[Int, Seq[Int]] =
      Source.fromFile(s"${arg.inputDir}closet.csv").getLines().map(line =>
        try {
          val fields = line.split(",")
          (fields(0).toInt, fields(1).toInt)
        } catch {
          case e: Exception => throw new RuntimeException(
            s"Cannot get user and item index from this line: ${line}. ${e}")
        }).toSeq.groupBy(x => x._1).mapValues(v => v.map(y => y._2))

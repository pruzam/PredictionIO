 ...
 */
object LTGenericDataPreparator {
  ...
  // LT-specific
  final val CLOSET_ADD: String = "add-to-closet"
  final val CLOSET_DEL: String = "remove-from-closet"
  final val CLOSET_RATING: Int = 4
  ...
  def dataPrep(arg: JobArg) = {
    ...
    val (builtInActions, closetActions) = u2iActions.filter(u2i =>
      usersMap.contains(u2i.uid) && itemsMap.contains(u2i.iid)
    ).partition(u2i =>
      isValidAction(u2i, arg.likeParam, arg.dislikeParam, arg.viewParam,
        arg.conversionParam))

    val builtInRatings = builtInActions.map(u2i => RatingData(
      uid = usersMap(u2i.uid),
      iid = itemsMap(u2i.iid).iindex,
      rating = convertToRating(u2i, arg.likeParam, arg.dislikeParam,
        arg.viewParam, arg.conversionParam),
      t = u2i.t.getMillis))

    val closetRatings = closetActions.groupBy(x => (x.iid, x.uid))
      .mapValues(v => v.reduce((a, b) => if (a.t > b.t) a else b)).values
      .filter(x => x.action == CLOSET_ADD).map(x => RatingData(
        uid = usersMap(x.uid),
        iid = itemsMap(x.iid).iindex,
        rating = CLOSET_RATING,
        t = x.t.getMillis))

    val filteredClosetRatings = closetRatings.filterNot(x =>
      builtInRatings.exists(y => x.uid == y.uid && x.iid == y.iid))

    val u2iRatings = builtInRatings ++ filteredClosetRatings
    ...
    /* write u2i seen */
    val u2iSeen = arg.seenActions.map { seenActions =>
      println(seenActions)
    ...
    /* write closet items */
    val unseenClosetRatings = u2iSeen.map(u2i =>
